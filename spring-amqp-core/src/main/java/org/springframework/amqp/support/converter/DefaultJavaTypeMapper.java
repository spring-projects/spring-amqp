...
public class DefaultJavaTypeMapper implements JavaTypeMapper, InitializingBean {
   ...
   public JavaType toJavaType(MessageProperties properties) {
      ...
      if (classType.getKeyType() == null) {
         return TypeFactory.collectionType((Class<? extends Collection>) classType.getRawClass(), contentClassType);
      }

      JavaType keyClassType = getClassIdType(retrieveHeader(properties, getKeyClassIdFieldName()));
      JavaType mapType = TypeFactory.mapType((Class<? extends Map>) classType.getRawClass(), keyClassType,
                                             contentClassType);
      return mapType;
   ...
   private JavaType getClassIdType(String classId) {
      ...
      try {
         return TypeFactory.type(ClassUtils.forName(classId, getClass().getClassLoader()));
      }
   ...
   public void fromJavaType(JavaType javaType, MessageProperties properties) {
      addHeader(properties, getClassIdFieldName(), (Class<?>) javaType.getRawClass());
