...
public abstract class AbstractAdaptableMessageListener implements ChannelAwareMessageListener {
	...
	private @Nullable String responseExchange;

	private @Nullable Address responseAddress;
	...
	private String listenerId = "rabbitListenerReply";
	...
	/**
	 * Set up the {@link ObservationRegistry} and listener id to use for observing reply messages.
	 * @param observationRegistry the observation registry.
	 * @param listenerId the listener id.
	 * @since 4.2
	 */
	public void setupObservationRegistry(ObservationRegistry observationRegistry, String listenerId) {
	 ...
	 * @see #sendResponse
	 */
	protected void handleResult(InvocationResult resultArg, Message request, @Nullable Channel channel) {
	...
	protected void handleResult(@Nullable InvocationResult resultArg, Message request,
		...
		if (resultArg != null) {
			CompletableFuture<?> completableFutureResult = null;
			if (resultArg.getReturnValue() instanceof CompletableFuture<?> completable) {
				...
				completableFutureResult = completable;
			}
				...
				completableFutureResult = MonoHandler.toCompletableFuture(resultArg.getReturnValue());
			}

			if (completableFutureResult != null) {
				...
				completableFutureResult.whenComplete((r, t) ->
					observation.scoped(() -> {
						if (t == null) {
							...
							asyncFailure(request, channel, cause, source);
						}
					}));
			}
			else {
	...
	protected @Nullable String getReceivedExchange(Message request) {
	 ...
	 * @since 2.3
	 */
	protected Message convert(@Nullable Object result, @Nullable Type genericType, MessageConverter converter) {
		...
		if (result == null) {
			...
			message = converter.toMessage(result, messageProperties, genericType);
		}
	 ...
	 */
	protected Address getReplyToAddress(Message request, @Nullable Object source, InvocationResult result) {
	...
	private Address evaluateReplyTo(Message request, @Nullable Object source, @Nullable Object result,
	...
	protected void sendResponse(@Nullable Channel channel, Address replyTo, Message messageIn) {
		...
		try {
			...
			if (this.logger.isDebugEnabled()) {
				this.logger.debug("Publishing response to exchange = [" + exchange + "], routingKey = ["
						+ routingKey + "]");
			}
			if (!this.observationRegistry.isNoop()) {
				...
				Observation replyObservation = RabbitTemplateObservation.TEMPLATE_OBSERVATION.observation(null,
								...
								() -> new RabbitMessageSenderContext(messageToSend, this.listenerId, exchange, routingKey),
