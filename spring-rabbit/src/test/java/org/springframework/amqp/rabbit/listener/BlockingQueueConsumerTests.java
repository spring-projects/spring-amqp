...
public class BlockingQueueConsumerTests {
	...
	public void testDrainAndReject() throws IOException, TimeoutException {
		...
		verify(channel).basicQos(2, false);
		Consumer consumer = TestUtils.<Map<String, Consumer>>propertyValue(blockingQueueConsumer, "consumers")
				.get("test");
		isOpen.set(false);
		blockingQueueConsumer.stop();
		verify(channel).basicCancel("consumerTag");
		...
		consumer.handleDelivery("consumerTag", envelope, props, new byte[0]);
		assertThat(TestUtils.<BlockingQueue<?>>propertyValue(blockingQueueConsumer, "queue")).hasSize(2);
		envelope = new Envelope(3, false, "foo", "bar");
		consumer.handleDelivery("consumerTag", envelope, props, new byte[0]);
		assertThat(TestUtils.<BlockingQueue<?>>propertyValue(blockingQueueConsumer, "queue")).isEmpty();
