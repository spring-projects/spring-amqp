...
Starting with version 4.2, listener reply messages are observed using `RabbitTemplateObservation.TEMPLATE_OBSERVATION` when observation is enabled, propagating distributed tracing context (such as `traceparent`) to reply messages.
For asynchronous listener return types (`CompletableFuture` or `Mono`), the listener receiver observation remains open until the asynchronous computation completes, ensuring trace context is propagated across threads to the reply message.
