...
Since `amqp-client` 5.33, hostname verification is enabled by default when TLS is configured via `useSslProtocol`.
The `enableHostnameVerification` property is deprecated (since 4.1) and has no effect.
