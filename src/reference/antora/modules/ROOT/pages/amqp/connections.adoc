...
When that property is `true`, the factory bean calls `ConnectionFactory#useTlsWithNoVerification()`, and the `sslAlgorithm` property is ignored (the client's default TLS protocol is used).
Since `amqp-client` 5.33, hostname verification is enabled by default whenever TLS is configured through `useSslProtocol`.
The `enableHostnameVerification` property is deprecated (since 4.2) and has no effect.
...
The framework provides the `WebFluxNodeLocator` and `RestClientNodeLocator` (since version 4.2), with the default as discussed above.
