...
NOTE: With the Jackson 3.x library, when converting from the `Message`, `MessageProperties.getContentType()` must be JSON-compatible (`"application/{asterisk}+json".isCompatibleWith(contentType)` is used by `MimeType` for this check).
