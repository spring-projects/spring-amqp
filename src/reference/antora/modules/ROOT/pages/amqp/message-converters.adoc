...
NOTE: With the Jackson 3.x library, when converting from the `Message`, an incoming `MessageProperties.getContentType()` must be JSON-compliant, `"application/{asterisk}+json".isCompatibleWith(contentType)` is used by `MimeType` to perform this check.
See https://github.com/spring-projects/spring-amqp/issues/3520[gh-3520]
