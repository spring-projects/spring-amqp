...
Starting with version 2.4.3, the converter will not add a `contentEncoding` message property if the `supportedContentType` has a `charset` parameter; this is also used for the encoding.
A new method `setSupportedContentType` has been added:
...
If neither exist, if the `supportedContentType` has a `charset` parameter, it will be used for decoding, with a final fallback to the `defaultCharset` property.
A new method `setSupportedContentType` has been added:
...
NOTE: When converting from the `Message`, an incoming `MessageProperties.getContentType()` must be JSON-compliant, `"application/{asterisk}+json".isCompatibleWith(contentType)` is used by `MimeType` to perform this check.
...
So, to meet the `JacksonJsonMessageConverter` requirements on the consumer side, the producer must add the `contentType` message property -- for example, as `application/json` or `application/vnd.api+json` or by using the `JacksonJsonMessageConverter`, which sets the header automatically.
...
IMPORTANT: Starting with version 1.6.11, `JacksonJsonMessageConverter` and, therefore, `DefaultJacksonJavaTypeMapper` (`DefaultClassMapper`) provide the `trustedPackages` option to overcome https://spring.io/security/cve-2017-4995[Serialization Gadgets] vulnerability.
