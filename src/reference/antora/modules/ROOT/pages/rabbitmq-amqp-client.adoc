...
Similar to the `AbstractRoutingConnectionFactory` for the AMQP 0.9.1 protocol (see xref:amqp/connections.adoc#routing-connection-factory[Routing Connection Factory]), the `spring-rabbitmq-client` module provides an `AbstractRoutingAmqpConnectionFactory` for the AMQP 1.0 protocol.
...
Otherwise, if there is no target for the `lookupKey` (or the key is `null`) and the factory is configured with `lenientFallback = true` (the default), the `defaultTargetConnectionFactory` is used.
If `lenientFallback = false` and no target matches a non-`null` key, an `IllegalStateException` is thrown.
...
Similar to the AMQP 0.9.1 listener containers, the `RabbitAmqpListenerContainer` is aware of a `RoutingAmqpConnectionFactory`.
When the container is configured with one, the `lookupKey` is bound to the current thread (via the `SimpleResourceHolder`) while the consumers' `Connection` is obtained, so the routing factory selects the appropriate target `AmqpConnectionFactory`.
The `lookupKey` is the comma-delimited list of the container queue names, bracketed by `[...]` - for example, `[q1,q2]`.
A `RabbitAmqpListenerContainer.setLookupKeyQualifier(String)` can be set to prefix this key, e.g. `tenant1[q1,q2]`, which is useful when the same queue names are used to address different brokers.
