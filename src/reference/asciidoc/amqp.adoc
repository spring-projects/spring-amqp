...
With versions prior to _version 2.0_, the was one listener container - the `SimpleMessageListenerContainer`; there is now a second container - the `DirectMessageListenerContainer`.
...
It's architecture is therefore actually "simpler" than the SMLC.
