...
Starting with _version 1.6_, if type information is missing, the converter will convert the JSON to a
`LinkedHashMap<String, Object>`, where the values might also be nested maps.
...
@RabbitListener
public void foo(Foo foo, Message message) {...}

@RabbitListener
public void foo(Foo foo, String bar) {...}
----

In the first three cases above the converter will attempt to convert to the `Foo` type.
The third example is invalid because we can't determine which argument should receive the message payload.
