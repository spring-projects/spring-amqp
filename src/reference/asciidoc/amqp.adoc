...
Starting with _version 1.6_, the naming strategy can be specified when using XML configuration; the strategy can
be `uuid`, `spring`, or a reference to a bean that implements `AnonymousQueue.NamingStrategy`.

[source, xml]
----
<rabbit:queue id="uuidAnon" naming-strategy="uuid" />

<rabbit:queue id="springAnon" naming-strategy="spring" />
...
The first creates names with a String representation of a UUID (the same as when no `naming-strategy` is provided).
...
Of course, you can provide your own naming strategy.
