...
Starting with version 2.2, `MessageListenerAdapter.buildListenerArguments(Object)` has been deprecated and provide `buildListenerArguments(Object, Channel, Message)`.
...
Now you can replace `MessageListenerAdapter` with `ExtendedListenerAdapter` in your configurations.
Arguments of listener should be set in matched position, as the following example shows:
