...
Starting with verdsion 2.1, `@RabbitListener` (and `@RabbitHandler` methods can be specified with asynchronous return types `ListenableFuture<?>` and `Mono<?>`).
The reply can then be sent asynchronously.
