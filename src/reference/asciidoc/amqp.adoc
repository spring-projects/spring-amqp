...
The listener with event keys, which are used to bind an `AnonymousQueue` to the exchange so the listener will receive only selected events; since it's a topic exchange, wildcards can be used.
