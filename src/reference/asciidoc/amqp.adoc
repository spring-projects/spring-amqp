...
While it is preferable to use `AnonymousQueues` as auto-delete queues, starting with _version 2.0.4_, you can use broker named queues with listener containers:
...
When the `RabbitAdmin` declares queues, it updates the `Queue.declaredName` property with the name returned by the broker.
...
The `Queue` that name is added to the `Queue` 's `declaredName` property.
...
Starting with _version 2.0.4_, listener containers can use queues of this type.
