...
===== RetryTemplate
Starting with version 3.1, you can now use  `RetryTemplate` to create `RetryableConnection` to support for retry mechanism at the connection level for creating channels to avoid the exception in connection module
====
[source, java]
----
connectionFactory.setRetryTemplate(new RetryTemplateBuilder().build());
----
====
