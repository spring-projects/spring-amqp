...
Also, as discussed in <<broker-configuration>>, the `RabbitTemplate` will re-declare any infrastructure beans (queues etc) when the connection is re-established.
...
Spring AMQP now uses the 4.0.x version of `spring-amqp`, which has auto recovery enabled by default.
Spring AMQP still uses its own recovery mechanisms so you can disable it in the client, if you wish, (by setting the `automaticRecoveryEnabled` property on the underlying `RabbitMQ connectionfactory` to `false`).
