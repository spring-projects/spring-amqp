...
Starting with version 2.0.6 a `RetryTemplate` and `RecoveryCallback` can be added to the listener container factory, used when sending replies.
The `RecoveryCallback` is invoked when retries are exhausted; a `SendRetryContextAccessor` can be used to get information from the context:
...
(The DMLC does have `messagesPerAck` which can be used to reduce the acks, the same as with `txSize` and the SMLC, but it can't be used with transactions - each message is delivered and ack'd in a separate transaction).
...
While it is preferable to use `AnonymousQueues` as auto-delete queues, starting with _version 2.1_, you can use broker named queues with listener containers:
...
You must use `setQueues()` when configuring the container, for this to work, so that the container can access the declared name at runtime, just setting the names is insufficient.

NOTE: You cannot add broker-named queues to the containers while they are running.

IMPORTANT: When a connection is reset and a new one established, the new queue will get a new name.
Since there is a race condition between the container restarting and the queue being re-declared, it's important to set the container's `missingQueuesFatal` property to `false` since it is likely the container will initially try to reconnect to the old queue.
...
Starting with version 2.0, the `Jackson2JsonMessageConverter` implements `SmartMessageConverter` which allows it to be used with the new `RabbitTemplate` methods that take a `ParameterizedTypeReference` argument; this allows conversion of complex generic types.
For example:
...
NOTE: Starting with version 2.1 the class `AbstractJsonMessageConverter` has been removed; it is no longer the base class for `Jackson2JsonMessageConverter`.
...
This class was introduced in _version 2.1_ and can be used to convert messages from/to XML.

Both of `Jackson2XmlMessageConverter` and `Jackson2JsonMessageConverter` have the same base class `AbstractJackson2MessageConverter`.

NOTE: The class `AbstractJackson2MessageConverter` is introduced to replace a removed class `AbstractJsonMessageConverter`.
...
_version 2.1.4_ introduced `addBeforePublishPostProcessor()` and `addAfterReceivePostProcessor()` to the `RabbitTemplate` to allow appending new post processor to the list of before publish and after receive post processors respectively. Also there are methods provided to remove the post processors.
See the Javadoc of `RabbitTemplate` for more detail.
...
That name is also added to the `Queue` 's `actualName` property.
This functionality can only be used programmatically by invoking the `RabbitAdmin` directly.
When using auto-declaration by the admin by defining a queue declaratively in the application context, set the name property to `""` and the broker will create the name.
Starting with _version 2.1_, listener containers can use queues of this type.
...
When the https://www.rabbitmq.com/event-exchange.html[Event Exchange Plugin] is enabled, if you add a bean of type `BrokerEventListener` to the application context, it will publish selected broker events as `BrokerEvent` s which can be consumed with a normal Spring `ApplicationListener` or `@EventListener` method.
...
The listener with event keys, which are used to bind an `AnonymousQueue` to the exchange so the listener will receive only selected events; since it's a topic exchange, wildcards can be used (as well as explicitly requesting specific events)
...
You can further narrow the received events in individual event listeners, using normal Spring techniques:
