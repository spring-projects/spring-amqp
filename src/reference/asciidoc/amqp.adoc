...
Starting with version 2.2, `buildListenerArguments(Object)` has been deprecated and provide `buildListenerArguments(Object, Channel, Message)`.
The new method treats not only `Object` but also helps listener to get `Channel` and `Message` arguments, such as getting `Channel` object to `basicReject(long, boolean)` in manual acknowledge mode.
...
Now you can replace `MessageListenerAdapter` with `ExtendedListenerAdapter` if you need to receive "`channel`" and "`message`".
Parameters of listener should be set same as `buildListenerArguments(Object, Channel, Message)` returned, as the following example of listener shows:
