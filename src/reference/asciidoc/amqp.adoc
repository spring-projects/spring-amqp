...
NOTE: When converting from the `Message`, an incoming `MessageProperties.getContentType()` must be JSON-compliant (`contentType.contains("json")` is used for logic).
Otherwise a `WARN` log message, like `Could not convert incoming message with content-type`, is emitted and `message.getBody()` is returned as is - in `byte[]` representation.
So, to meet a `Jackson2JsonMessageConverter` requirements on consumer side, the producer must add `contentType` message property, e.g. as `text/json`, or just use the same `Jackson2JsonMessageConverter`, if it possible.
