...
Starting with version 2.1.4, `addBeforePublishPostProcessor()` and `addAfterReceivePostProcessor()` have been added to the `RabbitTemplate` to allow appending new post processor to the list of before publish and after receive post processors respectively.
