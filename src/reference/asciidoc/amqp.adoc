...
Starting with _version 2.0_, the `org.springframework.amqp.rabbit.connection.Connection` can be supplied with the `com.rabbitmq.client.BlockedListener` s to to be notified for connection block and unblock events.
...
These can be useful in application logic to react appropriately to the problems on the Broker and perform some maintenance actions for example.

IMPORTANT: When single `CachingConnectionFactory` is used in the application, e.g. in case of auto-configuration by Spring Boot, it is important to understand that by default it provides a single shared connection instance.
And when it is blocked by the Broker, any its clients stop to work.
If we deal with producers and consumers in the same application, we may end up with the dead lock situation when producers are blocking connection because no resources on Broker any more and consumers can't free them because connection is blocked.
...
At the same time this isn't recommended if we use transactional producer and it is important to let that producer to reuse a `Channel` associated with the consumer transaction.
...
Starting with _version 2.0_, the `org.springframework.amqp.rabbit.connection.Connection` object can be supplied with the `com.rabbitmq.client.BlockedListener` s to to be notified for connection block and unblock events.
