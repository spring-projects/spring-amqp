...
Using Micrometer for observation is now supported, since version 3.0.5, for the `RabbitStreamTemplate` and stream listener container.
...
To add tags to timers/traces, configure a custom `RabbitStreamTemplateObservationConvention` or `RabbitStreamListenerObservationConvention` to the template or listener container (for native listeners), respectively.
To add tags to timers/traces for non-native listeners, configure a `RabbitListenerObservationConvention` to the listener container.

The default implementations add the `name` tag for template observations and `listener.id` tag for containers.

NOTE: When using a non-native listener (with message conversion, parameter resolution), the full name of the tag key is `spring.rabbit.listener.id`; for native listeners, it is `spring.rabbit.stream.listener.id`.
The reason for the difference is the context for native listeners uses a `com.rabbitmq.stream.Message` whereas non-native listeners use a `org.springframework.amqp.core.Message`.
